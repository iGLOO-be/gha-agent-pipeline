...
- `.github/actions/` — shared composites (`install-agent-pipeline`, `agent-phase-run` (post-setup orchestration), labels, comments, …) plus **`setup-pr-environment`** (local copy for dogfood only; other consumers keep their own)
